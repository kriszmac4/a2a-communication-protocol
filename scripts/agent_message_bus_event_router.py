...
Periodically scans the Agent Message Bus message bus for newly-completed (status='done')
...
Cron expression (suggested): every 5 minutes.
    */5 * * * *  cd /path/to/a2a-communication-protocol/scripts && python3 agent_message_bus_event_router.py >> /var/log/amb_router.log 2>&1

Outputs:
- Per-run: appended to skill_registry.json
- Notifications: sent via agent_message_bus_send_message to the orchestrator
- Logs: stdout (caller redirects to amb_router.log)

Usage:
    python3 agent_message_bus_event_router.py
    python3 agent_message_bus_event_router.py --once --lookback-minutes 60
    python3 agent_message_bus_event_router.py --dry-run
"""
...
ROUTER_LOG = Path(os.environ.get("AMB_ROUTER_LOG", Path.home() / ".a2a-protocol" / "amb_router.log"))
...
def _extract_skill_from_message(msg: dict) -> tuple[str | None, str | None]:
    ...
    1. The `result` field (from agent_mark_done) — e.g. "[skill=implement-feature] done"
    2. The `content` field (original message) — e.g. "[skill=implement-feature] build the API"
    3. Inference from from_agent/to_agent/result content

    Returns (agent_name, skill_id) or (from_agent, None) if no skill detected.
    """
    content = msg.get("content", "") or ""
    result = msg.get("result", "") or ""
    from_agent = msg.get("from_agent", "")

    # Helper: extract [skill=...] from a string
    def _parse_skill(text: str) -> str | None:
        if "[skill=" in text:
            try:
                start = text.index("[skill=") + 7
    ...
    # 1. Try result field first (most specific — agent_mark_done output)
    skill = _parse_skill(result)
    if skill:
        return from_agent, skill

    # 2. Try content field (original message from orchestrator)
    skill = _parse_skill(content)
    if skill:
        return from_agent, skill

    # 3. Try inferring skill from result content keywords
    #    (when agents don't use [skill=] convention but describe what they did)
    if result and from_agent:
        result_lower = result.lower()
        # Load the agent's card to find matching skill by keyword scoring
        try:
            cards = _load_all_agent_cards()
            card = cards.get(from_agent, {})
            best_match = None
            best_score = 0
            for s in card.get("skills", []):
                skill_id = s.get("id", "")
                keywords = s.get("keywords", [])
                score = sum(1 for kw in keywords if kw.lower() in result_lower)
                if score > best_score:
                    best_score = score
                    best_match = skill_id
            if best_match and best_score > 0:
                return from_agent, best_match
        except Exception:
            pass
...
def main():
    ...
    print(f"Agent Message Bus event router started. interval={args.interval}s", file=sys.stderr)
    while True:
        try:
            result = run_once(lookback_minutes=args.lookback_minutes, dry_run=args.dry_run)

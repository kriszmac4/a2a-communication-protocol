#!/usr/bin/env python3
"""
AMB Push Notifier — sends a real-time notification to the target agent's
Discord channel when a new message lands on the AMB.
...
AMB_DB = HERMES_HOME / "data" / "agent_message_bus" / "agent_messages.db"
...
def _get_db() -> sqlite3.Connection:
    """Open a direct SQLite connection to the AMB DB (no AMB import)."""
    AMB_DB.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(str(AMB_DB))
...
def _save_discord_message_id(msg_id: int, discord_message_id: str):
    """Store the Discord message ID for an AMB bus message."""
    try:
        conn = _get_db()
        conn.execute(
...
def send_notification(to_agent: str, from_agent: str, msg_id: int,
    ...
    msg = f"{priority_mark}📬 **AMB** — {from_emoji} **{from_agent}** → {to_emoji} **{to_agent}**"
...
# =============================================================================
# CLI: start-thread mode (delegates to agent_message_bus.open_message_thread)
# =============================================================================

def start_thread(msg_id: int):
    """Open a thread for an existing notification via agent_message_bus.open_message_thread()."""
    try:
        sys.path.insert(0, str(Path.home() / ".hermes" / "scripts"))
        from agent_message_bus import open_message_thread as _open_thread
        success = _open_thread(msg_id)
        if success:
            print(f"[notify_target] ✅ #{msg_id} thread opened via amb", file=sys.stderr)
        else:

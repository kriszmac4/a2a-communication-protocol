...
AGENT_PERSONA = (
    ...
    "Türelmesen és didaktikusan tanítasz, példákon keresztül. "
...
def query_llm(user_message: str) -> str | None:
    ...
    try:
        with urllib.request.urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
            ...
            return f"❌ Study Bridge: üres válasz"
    except urllib.error.HTTPError as e:
        body = e.read().decode("utf-8", errors="replace")[:200]
        return f"❌ Study Bridge HTTP {e.code}: {body}"
    except urllib.error.URLError as e:
        return f"❌ Study Bridge hálózati hiba: {e.reason}"
    except Exception as e:
        return f"❌ Study Bridge ismeretlen hiba: {type(e).__name__}: {e}"
    ...
    try:
        ...
        for msg in messages:
            ...
            if llm_response and not llm_response.startswith("❌"):
                ...
                print(f"🤖 #{msg_id} {from_agent}→{AGENT_ID}: Study LLM válasz (depth={new_depth}) {msg_label}")
            else:
        ...
        if responses_sent > 0:
            ...
            print(f"\n📬 **STUDY Bridge — {responses_sent} {plural} elküldve**")
